...
const categories = new Set(['Internship', 'Hackathon', 'Competition', 'Fellowship', 'Scholarship', 'Quiz', 'Workshop', 'Certification']);
...
export class ValidationService {
  validate(intent: ActionIntent | null, entity: ActionEntity | null, payload: ExtractedPayload): ActionValidation {
    ...
    this.enum(payload, 'priority', priorities, issues); this.enum(payload, 'status', statuses, issues); this.enum(payload, 'category', categories, issues);
