...
/** Cerebras implementation of Nova's existing provider contract. */
export class CerebrasProvider implements AIProvider {
  ...
  async *streamChat(request: NovaChatRequest): AsyncGenerator<string, void, undefined> {
    ...
    if (!apiKey) throw new GeminiServiceError('missing-api-key', 'Cerebras is not configured.', false);
    ...
    if (!receivedText) throw new GeminiServiceError('invalid-response', 'Cerebras returned an empty response.', false);
  }
  ...
  private toServiceError(error: unknown): GeminiServiceError {
    ...
    const message = error instanceof Error ? error.message : 'Cerebras could not complete the request.';
