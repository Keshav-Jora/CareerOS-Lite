...
export default function AdminConsole() {
  ...
  return <main className="min-h-screen bg-slate-950 px-4 py-8 text-slate-100 sm:px-8 lg:px-12"><div className="mx-auto max-w-7xl">
    ...
    {error && <div role="alert" className="mb-6 flex flex-col gap-3 rounded-xl border border-rose-900/70 bg-rose-950/30 p-4 text-sm text-rose-200 sm:flex-row sm:items-center sm:justify-between"><span className="flex items-center gap-3"><AlertCircle className="h-5 w-5 shrink-0" />Unable to load Owner Console data: {error}</span><button type="button" onClick={() => void loadAnalytics()} className="rounded-lg border border-rose-800 px-3 py-1.5 text-xs font-medium hover:bg-rose-900/40">Try again</button></div>}
